...
if __name__ == "__main__":
    ...
    envs = make_vec_envs(env_type, env_name, 
                        seed, args.num_processes,
                        args.gamma, log_folder, device, False)
    ...
    for j in range(num_updates):
        ...
        if j % v['eval']['log_interval'] == 0:
            ...
            eval_episode_rewards, eval_episode_steps = evaluate(
                actor_critic, envs.obs_rms, env_type, env_name, seed,
                args.num_processes, log_folder, device, num_eval_tasks
            )

            if env_type == "pomdp":
                logger.record_tabular('metrics/total_steps_eval', np.mean(eval_episode_steps))
                logger.record_tabular('metrics/return_eval_total', np.mean(eval_episode_rewards))
                perf = np.mean(eval_episode_rewards)
            elif env_type == "rmdp":
                cutoff = np.percentile(eval_episode_rewards, 100 * worst_percentile)
                worst_indices = np.where(eval_episode_rewards <= cutoff) # must be "<=" to avoid empty set
                returns_eval_worst, total_steps_eval_worst = eval_episode_rewards[worst_indices], eval_episode_steps[worst_indices]
                
                logger.record_tabular('metrics/return_eval_avg', eval_episode_rewards.mean())
                logger.record_tabular('metrics/return_eval_worst', returns_eval_worst.mean())
                logger.record_tabular('metrics/total_steps_eval_avg', eval_episode_steps.mean())
            ...
            if j % v['eval']['save_interval'] == 0 and j > 0.75 * num_updates:
                ...
                ], os.path.join(logger.get_dir(), 'save',
                    f"agent_{j}_perf{perf:.3f}.pt"))
