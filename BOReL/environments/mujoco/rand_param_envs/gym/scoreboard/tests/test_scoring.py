...
def _assert_benchmark_score(
    scores,
    score=None,
    num_envs_solved=None,
    summed_training_seconds=None,
    summed_task_wall_time=None,
    start_to_finish_seconds=None,
):
    ...
    if summed_training_seconds is not None:
        assert _is_close(
            scores["summed_training_seconds"], summed_training_seconds
        ), debug_str
    if summed_task_wall_time is not None:
        assert _is_close(
            scores["summed_task_wall_time"], summed_task_wall_time
        ), debug_str
    if start_to_finish_seconds is not None:
        assert _is_close(
            scores["start_to_finish_seconds"], start_to_finish_seconds
        ), debug_str
...
def test_clip_average_evaluation_scoring():
    ...
    benchmark_result = _benchmark_result_helper(
        benchmark, episode_rewards=[100, 100], episode_lengths=[1, 1]
    )
...
def test_clip_average_benchmark_scoring():
    ...
    for i, task in enumerate(benchmark.tasks):
        ...
        benchmark_results[env_id].append(
            _benchmark_result_helper(benchmark, env_id=env_id, timestamps=[i + 2])
        )
...
def test_clip_average_benchmark_incomplete():
    ...
    benchmark_results[env_id].append(
        _benchmark_result_helper(benchmark, env_id=env_id, timestamps=[2])
    )
    scores = scoring.benchmark_aggregate_score(benchmark, benchmark_results)
    _assert_benchmark_score(
        scores,
...
def test_clip_average_benchmark_extra():
    ...
    benchmark_results[env_id].append(
        _benchmark_result_helper(
            benchmark, env_id=env_id, episode_rewards=[100], timestamps=[2]
        )
    )
...
def test_clip_average_benchmark_eval_handling():
    ...
    _assert_benchmark_score(
        ...
        score=0.0004,
        num_envs_solved=0,
        summed_training_seconds=5.0,
        summed_task_wall_time=5.0,
        start_to_finish_seconds=3.0,
    )
...
def test_total_reward_benchmark_scoring():
    ...
    for i, task in enumerate(reward_benchmark.tasks):
        ...
        benchmark_results[env_id].append(
            _benchmark_result_helper(
                reward_benchmark, env_id=env_id, timestamps=[i + 2]
            )
        )
...
def test_total_reward_benchmark_empty():
    ...
    env_id = task.env_id
    benchmark_results[env_id].append(
        _benchmark_result_helper(reward_benchmark, env_id=env_id)
    )
    scores = scoring.benchmark_aggregate_score(reward_benchmark, benchmark_results)
...
def test_total_reward_benchmark_solved():
    ...
    for i, task in enumerate(reward_benchmark.tasks):
        env_id = task.env_id
        benchmark_results[env_id].append(
            reward_benchmark.score_evaluation(
                env_id,
                data_sources=[0] * N,
                initial_reset_timestamps=[1],
                episode_lengths=[1] * N,
    ...
    scores = scoring.benchmark_aggregate_score(reward_benchmark, benchmark_results)
    _assert_benchmark_score(
        scores, score=1.0, num_envs_solved=len(reward_benchmark.tasks)
    )
...
def test_benchmark_incomplete():
    ...
    benchmark_results[env_id].append(
        _benchmark_result_helper(reward_benchmark, env_id=env_id, timestamps=[2])
    )
    scores = scoring.benchmark_aggregate_score(reward_benchmark, benchmark_results)
    _assert_benchmark_score(
        scores,
...
def test_benchmark_extra():
    ...
    benchmark_results[env_id].append(
        _benchmark_result_helper(
            reward_benchmark, env_id=env_id, episode_rewards=[100], timestamps=[2]
        )
    )
...
def test_benchmark_eval_handling():
    ...
    _assert_benchmark_score(
        ...
        score=0.02,
        num_envs_solved=0,
        summed_training_seconds=8.0,
        summed_task_wall_time=7.0,
        start_to_finish_seconds=4.0,
    )
...
def test_reward_per_time_benchmark_scoring():
    ...
    for i, task in enumerate(reward_per_time_benchmark.tasks):
        ...
        benchmark_results[env_id].append(
            _benchmark_result_helper(
                reward_per_time_benchmark, env_id=env_id, timestamps=[i + 2]
            )
        )
    scores = scoring.benchmark_aggregate_score(
        reward_per_time_benchmark, benchmark_results
    )
...
def test_reward_per_time_benchmark_empty():
    ...
    benchmark_results[env_id].append(
        _benchmark_result_helper(
            reward_per_time_benchmark, env_id=env_id, episode_lengths=[10]
        )
    )
    scores = scoring.benchmark_aggregate_score(
        reward_per_time_benchmark, benchmark_results
...
def test_reward_per_time_benchmark_solved():
    ...
    for i, task in enumerate(reward_per_time_benchmark.tasks):
        env_id = task.env_id
        benchmark_results[env_id].append(
            reward_per_time_benchmark.score_evaluation(
                env_id,
                data_sources=[0] * N,
                initial_reset_timestamps=[1],
                episode_lengths=[1] * N,
