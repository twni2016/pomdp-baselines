...
try:
    ...
    raise error.DependencyNotInstalled(
        '{}. (HINT: you may need to install the Go dependencies via "pip install gym[pachi]".)'.format(
            e
        )
    )
...
class GoState(object):
    ...
    def __repr__(self):
        return "To play: {}\n{}".format(
            six.u(pachi_py.color_to_str(self.color)), self.board.__repr__().decode()
        )
...
def make_pachi_policy(board, engine_type="uct", threads=1, pachi_timestr=""):
    ...
    def pachi_policy(curr_state, prev_state, prev_action):
        if prev_state is not None:
            assert (
                engine.curr_board == prev_state.board
            ), "Engine internal board is inconsistent with provided board. The Pachi engine must be called consistently as the game progresses."
...
def _play(black_policy_fn, white_policy_fn, board_size=19):
    ...
    while not curr_state.board.is_terminal:
        a = (black_policy_fn if curr_state.color == BLACK else white_policy_fn)(
...
class GoEnv(gym.Env):
    ...
    def __init__(
        self, player_color, opponent, observation_type, illegal_move_mode, board_size
    ):
        """
        Args:
    ...
    def _step(self, action):
        ...
        if self.done:
            return self.state.board.encode(), 0.0, True, {"state": self.state}

        # If resigned, then we're done
        if action == _resign_action(self.board_size):
            self.done = True
            return self.state.board.encode(), -1.0, True, {"state": self.state}
        ...
        if not self.state.board.is_terminal:
            self.done = False
            return self.state.board.encode(), 0.0, False, {"state": self.state}
        ...
        reward = 1.0 if player_wins else -1.0 if (white_wins or black_wins) else 0.0
    ...
    def _reset_opponent(self, board):
        if self.opponent == "random":
            ...
            self.opponent_policy = make_pachi_policy(
                board=board, engine_type=six.b("uct"), pachi_timestr=six.b("_2400")
            )  # TODO: strength as argument
        else:
