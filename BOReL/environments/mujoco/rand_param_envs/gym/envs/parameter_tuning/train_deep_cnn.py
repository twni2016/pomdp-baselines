...
class CNNClassifierTraining(gym.Env):
    ...
    def _step(self, action):
        ...
        diverged, acc = self.train_blueprint(
            lr, decay, momentum, batch_size, l1, l2, convs, fcs
        )
    ...
    def _get_obs(self):
        ...
        return np.array([self.nb_inst, self.previous_acc])
    ...
    def train_blueprint(self, lr, decay, momentum, batch_size, l1, l2, convs, fcs):
        ...
        if has_convs:
            ...
            model.add(
                Flatten(input_shape=(img_channels, img_rows, img_cols))
            )  # avoid excetpions on no convs
        ...
        H = model.fit(X, Y, batch_size=int(batch_size), nb_epoch=10, shuffle=True)
