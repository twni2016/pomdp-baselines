...
class ConvergenceControl(gym.Env):
    ...
    def _step(self, action):
        ...
        H = self.model.fit(X, Y, batch_size=int(batch_size), nb_epoch=1, shuffle=True)
    ...
    def _reset(self):
        ...
        if random.choice([True, True, False]):
            ...
            model.add(
                Convolution2D(self.convAsz, 3, 3, W_regularizer=reg, b_regularizer=reg)
            )
            model.add(Activation("relu"))
            ...
            if self.convBsz > 0:
                ...
                model.add(
                    Convolution2D(
                        self.convBsz, 3, 3, W_regularizer=reg, b_regularizer=reg
                    )
                )
                model.add(Activation("relu"))
        ...
        model.add(Dense(self.densesz, W_regularizer=reg, b_regularizer=reg))
        model.add(Activation("relu"))
