...
from environments.mujoco.rand_param_envs.gym.envs.classic_control.cartpole import (
    CartPoleEnv,
)
...
class PredictObsCartpoleEnv(Env):
    def __init__(self):
        ...
        self.action_space = spaces.Tuple(
            (self.cartpole.action_space,)
            + (self.cartpole.observation_space,) * (NUM_PREDICTED_OBSERVATIONS)
        )
    ...
    def _step(self, action):
        ...
        if not done:
            ...
            if self.iteration > TIME_BEFORE_BONUS_ALLOWED:
                for i in xrange(
                    min(NUM_PREDICTED_OBSERVATIONS, len(self.predicted_observations))
                ):
                    l2dist = np.sqrt(
                        np.sum(
                            np.square(
