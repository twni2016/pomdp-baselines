...
class AntSemiCircleEnv(MultitaskAntEnv):
    ...
    def step(self, action):
        ...
        ctrl_cost = 0.1 * np.square(action).sum()  # NOTE: <= 0.1*8
    ...
    def reset_pos(self, modify_init_state_dist=False, on_circle_init_state=False):
        # NOTE: used for evaluation on fixed position
        qpos = self.init_qpos
        # just for offline data collection:
        if modify_init_state_dist:
            qpos[:2] = np.array(
                [np.random.uniform(-1.5, 1.5), np.random.uniform(-0.5, 1.5)]
            )
            if (
                not on_circle_init_state
            ):  # make sure initial state is not on semi-circle
                # while 1 - self.goal_radius <= np.linspace.norm(qpos[:2]) <= 1 + self.goal_radius:
                while (
                    0.8 <= np.linalg.norm(qpos[:2]) <= 1.2
    ...
    def _get_obs(self):
        return np.concatenate(
            [
                self.sim.data.qpos.flat,  # NOTE: add pos x,y. and self.get_body_com("torso") is just pos x,y,z
...
class SparseAntSemiCircleEnv(AntSemiCircleEnv):
    ...
    def sparsify_rewards(self, d):
        ...
        return non_goal_rewards + sparse_goal_reward  # [-0.8, 1]
