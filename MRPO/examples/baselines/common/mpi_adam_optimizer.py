...
class MpiAdamOptimizer(tf.train.AdamOptimizer):
    ...
    def compute_gradients(self, loss, var_list, **kwargs):
        ...
        flat_grad = (
            tf.concat([tf.reshape(g, (-1,)) for g, v in grads_and_vars], axis=0)
            * self.mpi_rank_weight
        )
        ...
        self.comm.Allreduce(
            np.array([self.mpi_rank_weight], dtype=np.float32), total_weight, op=MPI.SUM
        )
...
def test_nonfreeze():
    ...
    update_op = MpiAdamOptimizer(comm=MPI.COMM_WORLD, learning_rate=stepsize).minimize(
        loss
    )
